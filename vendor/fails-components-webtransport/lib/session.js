...
// Discord Locutor fork seam. The native transport calls HttpWTSession.onClose
// after an application may already have cancelled an aggregate reader. Node's
// ReadableStream controllers reject a second close with ERR_INVALID_STATE, so
// the effect must be both observable for the live oracle and idempotent.
let onCloseEffectHook = null;
const terminalReadableControllers = new WeakSet();

/**
 * Installs a synchronous hook around the native onClose controller effect.
 * The hook receives an idempotent effect function and may retain it for a
 * deterministic late release. Passing null restores the normal path.
 *
 * @param {null | ((entry: {session: HttpWTSession, effect: () => void}) => void)} hook
 * @returns {() => void}
 */
export function setOnCloseEffectHook(hook) {
  if (hook !== null && hook !== undefined && typeof hook !== 'function') {
    throw new TypeError('onClose effect hook must be a function or null');
  }
  const previous = onCloseEffectHook;
  onCloseEffectHook = hook ?? null;
  return () => {
    if (onCloseEffectHook === hook) onCloseEffectHook = previous;
  };
}

/**
 * @param {ReadableStreamDefaultController | ReadableByteStreamController | undefined} controller
 */
function closeReadableController(controller) {
  if (!controller || terminalReadableControllers.has(controller)) return;
  terminalReadableControllers.add(controller);
  controller.close();
}
 ...
 * @param {ReadableStreamDefaultController | ReadableByteStreamController | undefined} controller
 */
function markReadableControllerCancelled(controller) {
  if (controller) terminalReadableControllers.add(controller);
}

/**
 * @param {HttpWTSession} session
 */
function closeIncomingControllers(session) {
  closeReadableController(session.incomBiDiController);
  closeReadableController(session.incomUniDiController);
  closeReadableController(
    session.incomDatagramController_ || session.incomDatagramControllerBytes_,
  );
}
...
export class HttpWTSession {
  ...
  constructor(args) {
    ...
    this.incomingBidirectionalStreams = new ReadableStream({
      ...
      cancel: () => markReadableControllerCancelled(this.incomBiDiController),
    });
    ...
    this.incomingUnidirectionalStreams = new ReadableStream({
      ...
      cancel: () => markReadableControllerCancelled(this.incomUniDiController),
    });
      ...
      start: (
        ...
        if (this.datagramsReadableMode_ === 'bytes') {
          ...
          this.incomDatagramController_ =
            /** @type {import("stream/web").ReadableStreamController<Uint8Array>} */ (controller);
        }
      },
      cancel: () =>
        markReadableControllerCancelled(
    ...
    this.datagrams = {
      ...
      createWritable: (options) => {
        ...
        // Discord Locutor fork seam. QUICHE distinguishes "accepted" from
        // "not sent": `blocked` means its congestion queue is already full and
        // `tooBig` means the datagram cannot fit in one UDP packet. The
        // upstream wrapper used to resolve `writer.write()` for both outcomes,
        // making a live-media sender count silent loss as successful delivery.
        //
        // Do not reject the WritableStream here: a sink rejection permanently
        // errors that writer and prevents the recovery keyframe from restoring
        // the lane. Publish the per-writer native result instead, so consumers
        // that need freshness can drop the frame while keeping the writer
        // reusable. Consumers unaware of this fork seam retain upstream
        // behavior.
        ...
        const retWritable = new WritableStream({
          ...
          write: (chunk, controller) => {
            ...
            if (chunk instanceof Uint8Array) {
              ...
              lastWriteStatus = { code, message };
              if (code !== 'success' && code !== 'blocked' && code !== 'tooBig') {
  ...
  createBidirectionalStream(opts) {
    ...
    if (!notblocked) {
      ...
      if (rej) rej(new DOMException('No streams available', 'QuotaExceededError'));
    }
    return prom;
  }
   ...
   */
  createSendGroup() {
    if (this.state === 'closed' || this.state === 'failed') throw new Error('InvalidState');
  ...
  onClose(args) {
    ...
    if (this.state !== 'connected') {
      log.error('session was closed before state was "connected" - it was "%s"', this.state);
      this.state = 'failed';
    ...
    const controllerEffect = () => closeIncomingControllers(this);
    if (onCloseEffectHook) onCloseEffectHook({ session: this, effect: controllerEffect });
    else controllerEffect();
  ...
  onDatagramReceived(args) {
    ...
    if (args.datagram.byteLength === 0 && this.datagramsReadableMode_ === 'bytes') {
    ...
    if (this.incomDatagramControllerBytes_?.byobRequest) {
      ...
      // @ts-ignore
      if (!(view instanceof Uint8Array)) throw new Error('byob view is not a Uint8Array');
      if (view.byteLength < args.datagram.byteLength) {
        throw new Error('supplied view is not large enough.');
      }
      const destview = new Uint8Array(view.buffer, 0 + view.byteOffset, args.datagram.byteLength);
      destview.set(args.datagram);
      byob.respond(args.datagram.byteLength);
    } else {
      // @ts-ignore
      (this.incomDatagramController_ || this.incomDatagramControllerBytes_).enqueue(
        new Uint8Array(args.datagram),
      );
    }
  }
